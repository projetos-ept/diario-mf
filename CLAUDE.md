...
- **Impressão do HTML oficial exportado**: o exportador embute todo o CSS da página (`collectCss()`), então qualquer regra de impressão que dependa da estrutura do app (`main`, `#tab-official`, `.panel`) precisa ser escrita de forma a não esconder nada no arquivo exportado. Em particular, `body>*:not(main)` só pode existir sob `body.print-official`, e o HTML exportado envolve as folhas em `<main class="official-export">` — sem isso a pré-visualização de impressão do arquivo sai em branco.
